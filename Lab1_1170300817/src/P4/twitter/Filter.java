...
public class Filter {
	...
	public static List<Tweet> writtenBy(List<Tweet> tweets, String username) {
		List<Tweet> sortByUser = new ArrayList<Tweet>();
		for (Tweet t : tweets) {
	...
	public static List<Tweet> containing(List<Tweet> tweets, List<String> words) {
		List<Tweet> tweetsContainingSpecialWords = new ArrayList<Tweet>();
		for (Tweet t : tweets) 
		{
			String[] splitText = t.getText().split("\\s+");
			List<String> splitTextList = Arrays.asList(splitText);
			for (String w : words) {
				if (splitTextList.contains(w)) {
					tweetsContainingSpecialWords.add(t);
					break;
				}
			}
		}
		return tweetsContainingSpecialWords;
	}
}
